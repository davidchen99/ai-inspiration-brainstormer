...
点子卡片会显示方案、正文、图片、视频脚本的资产状态，并给出下一步建议。可一键选择当前筛选结果，方便从浏览进入批量处理。
...
批量文字和视频脚本生成会在“批量导出与高级操作”顶部显示统一任务反馈。

- 显示当前任务类型、处理进度、成功、失败和跳过数量。
- 生成完整正文前如果需要自动补齐方案，会先显示补齐状态。
- 批量按钮被锁定时，会提示当前正在处理或需要先选择点子。
- 批量完成后会提示下一步，例如继续生成正文、导出文档或导出视频任务包。
- 批量操作按意图分为生成方案、生成正文、生成图片/视频、导出结果和高级扩展。
...
- “只导出正文”：生成正文包，`正文/` 只包含已有完整正文，`草稿构思/` 保存方案、大纲和过程材料，根目录包含导出清单；无正文的点子只进入 `草稿构思/` 并在清单里标记为“无正文”。
